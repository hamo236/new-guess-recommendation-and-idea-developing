...
if (!context.includes('teamResolutionInFlightRef') || !context.includes('hasResolvableTeamConfirmation(state)') || !context.includes('resolveTeamRound()')) throw new Error('Provider must automatically resolve a complete Team Battle confirmation pair.');
...
if (!page.includes('const TEAM_LOBBY_SEATS = 3') || (!page.includes('Array.from({ length: TEAM_LOBBY_SEATS }') && !page.includes('Array.from({ length: Math.max(TEAM_LOBBY_SEATS, team.players.length)')) || !page.includes('{team.players.length}/{TEAM_LOBBY_SEATS} READY')) throw new Error('2v2 lobby must expose three selectable seats per team.');

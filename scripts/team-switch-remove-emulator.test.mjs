...
try {
  ...
  await assertSucceeds(player.ref('teamRooms/tm-1/teamSeats/team_b_2').set({ playerId: 'player-b' }));
  ...
  await assertSucceeds(player.ref('teamRooms/tm-1/joinSlots/slot-2').set({ playerId: 'player-b', joinOrder: 2 }));
  console.log(JSON.stringify({ status: 'passed', scopedTeamSwitch: true, protectedFields: 'blocked', outsiderWrite: 'blocked', hostRemoval: true, releasedSlotReusable: true }, null, 2));
} finally {
